...
class Numbers 
{
	public static function numtoLetter($xcifra)
	{ 
		$xarray = array(0 => "Cero",
					...
					switch ($xy) {
						case 1: // checa las centenas
							if (substr($xaux, 0, 3) < 100) { // si el grupo de tres dígitos es menor a una centena ( < 99) no hace nada y pasa a revisar las decenas
								...
								if (array_key_exists(substr($xaux, 0, 3), $xarray)) {
									$xseek = $xarray[substr($xaux, 0, 3)];
									$xsub = Numbers::subfijo($xaux); // devuelve el subfijo correspondiente (Millón, Millones, Mil o nada)
						...
						case 2: // checa las decenas (con la misma lógica que las centenas)
							if (substr($xaux, 1, 2) < 10) {
								...
								if (array_key_exists(substr($xaux, 1, 2), $xarray)) {
									$xseek = $xarray[substr($xaux, 1, 2)];								
									$xsub = Numbers::subfijo($xaux);
